...
public class tekileft_2 : MonoBehaviour
{
    public GameObject tekitama2;
    ...
    void Start()
    {
        this.gameObject.transform.position = new Vector3(-7, 4, 0);
    ...
    void Update()
    {
        ...
        if (time > 3)
        {
            ...
            if (MaxCount < count)
            {
                temp = Instantiate(tekitama2, gameObject.transform.position, Quaternion.identity);
                temp.GetComponent<tekitama2>().v = new Vector3(UnityEngine.Random.Range(0, 10), UnityEngine.Random.Range(-10, 10), 0);
                count = 0;
            }
            if (HEALcount < hcount)
            {

...
public class tekileft : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (time > 3)
        {
            ...
            if (MaxCount < count)
            {
                ...
                temp.GetComponent<ryuutama>().v = new Vector3(UnityEngine.Random.Range(0, 10), UnityEngine.Random.Range(-6, 6), 0);
                count = 0;
            }
            if (HEALcount < hcount)
            {
                htemp = Instantiate(heal, gameObject.transform.position, Quaternion.identity);
                htemp.GetComponent<heal>().v = new Vector3(UnityEngine.Random.Range(3, 10), UnityEngine.Random.Range(-6, 6), 0);
                hcount = 0;
            }
        }
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (other.gameObject.layer == 18)
        {
            ...
            {
                Application.LoadLevel("BeforeROUND3tiger");
                // オーディオを再生
                AudioSource.PlayClipAtPoint(explosionSE, transform.position);
                // ミサイルオブジェクトを破棄
